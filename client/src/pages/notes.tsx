...
`;
  ...
  const handleAIEnhancement = (action: string) => {
    // Use selected text if available, otherwise use the entire sermon body or theme as fallback
    let textToEnhance = selectedText;
    
    if (!textToEnhance && sermonBody) {
      textToEnhance = sermonBody;
    } else if (!textToEnhance && sermonTheme) {
      textToEnhance = sermonTheme;
    } else if (!textToEnhance) {
      textToEnhance = "Jesus loves you"; // Minimal fallback to allow enhancement
    }
    
    aiEnhanceMutation.mutate({ action, text: textToEnhance });
  };
                    ...
                    <CardHeader>
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
                        ...
                        >
                          ...
                          {selectedText && <div className="text-xs text-blue-300 mt-1">({selectedText.length} chars)</div>}
                        </div>
                        
                        <div
                          onClick={() => handleAIEnhancement("rewrite")}
                          className={`p-2 rounded-lg border cursor-pointer transition-all text-center ${
                            aiEnhanceMutation.isPending 
                              ? "border-gray-600 text-gray-500 cursor-not-allowed" 
                        ...
                        >
                          ...
                          {selectedText && <div className="text-xs text-green-300 mt-1">({selectedText.length} chars)</div>}
                        </div>
                        
                        <div
                          onClick={() => handleAIEnhancement("add_verse")}
                          className={`p-2 rounded-lg border cursor-pointer transition-all text-center ${
                            aiEnhanceMutation.isPending 
                              ? "border-gray-600 text-gray-500 cursor-not-allowed" 
                        ...
                        >
                          ...
                          {selectedText && <div className="text-xs text-purple-300 mt-1">({selectedText.length} chars)</div>}
                        </div>
                        
                        <div
                          onClick={() => handleAIEnhancement("add_illustration")}
                          className={`p-2 rounded-lg border cursor-pointer transition-all text-center ${
                            aiEnhanceMutation.isPending 
                              ? "border-gray-600 text-gray-500 cursor-not-allowed" 
                        ...
                        >
                          ...
                          {selectedText && <div className="text-xs text-orange-300 mt-1">({selectedText.length} chars)</div>}
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
